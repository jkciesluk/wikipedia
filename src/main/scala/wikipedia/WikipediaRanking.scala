...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)(
    (acc, article) => if(article.mentionsLanguage(lang)) acc + 1 else acc, _+_)
   ...
   *   several seconds.
   */
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = langs.map((lang) => (lang, occurrencesOfLang(lang, rdd))).sortWith(_._2 > _._2)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    ...
    index.reduceByKey(_ + _).collect().toList.sortWith(_._2 > _._2)
  }
